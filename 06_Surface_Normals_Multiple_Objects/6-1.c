...
float hit_sphere(float *center, float radius, ray *r);
...
ray_color(ray *r, float *dest)
{
    ...
    t = hit_sphere((vec3){0.0f, 0.0f, -1.0f}, 0.5f, r);

    if (t > 0.0f) {
        ray_at(r, t, temp);
        glm_vec3_sub(temp, (vec3){0.0f, 0.0f, -1.0f}, temp);
...
float
hit_sphere(float *center, float radius, ray *r)
{
    float discriminant;
    float a, b, c;
    vec3 oc = GLM_VEC3_ZERO_INIT;

    glm_vec3_sub(r->origin, center, oc);

    a = glm_vec3_dot(r->direction, r->direction);
    b = 2.0f * glm_vec3_dot(oc, r->direction);
    c = glm_vec3_dot(oc, oc) - radius * radius;
