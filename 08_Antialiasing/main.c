...
render(hittable_list *hl, camera *cam)
{
    ...
    float px, py;
    float fr, fg, fb;
    vec3 pixel_color, pixel_center, pixel_sample, temp;

    ray r = {GLM_VEC3_ZERO_INIT, GLM_VEC3_ZERO_INIT};
    ...
    for (i = 0; i < cam->image_height; i++) {
        for (j = 0; j < cam->image_width; j++) {
            ...
            for (sample = 0; sample < cam->samples_per_pixel; sample++) {
                ...
                /* ps = pc + (pdu * px + pdv * py) */
                px = -0.5f + rand() / (RAND_MAX + 1.0f);
                py = -0.5f + rand() / (RAND_MAX + 1.0f);

                glm_vec3_scale(cam->pixel_delta_u, px, temp);
                glm_vec3_muladds(cam->pixel_delta_v, py, temp);
                glm_vec3_add(pixel_center, temp, pixel_sample);

                glm_vec3_copy(cam->center, r.origin);
                glm_vec3_sub(pixel_sample, r.origin, r.direction);

                ray_color(&r, hl, temp);
                glm_vec3_add(pixel_color, temp, pixel_color);
            }
            ...
            scale = 1.0f / cam->samples_per_pixel;
            fr *= scale;
            fg *= scale;
            fb *= scale;

            ir = (int)(256 * glm_clamp(fr, 0.0f, 0.999f));
            ig = (int)(256 * glm_clamp(fg, 0.0f, 0.999f));
            ib = (int)(256 * glm_clamp(fb, 0.0f, 0.999f));
